...
class NN(nn.Module):
    ...
    def dotproduct1(self, x, h, batch):
        ...
        attn = F.softmax(torch.matmul(self.dotline1(x), self.dotline1(x).reshape([batch, h, length])) / math.sqrt(h), dim=1)
        y = torch.matmul(attn, self.dotline1(x))

        # print
        if i == 299:
            # attn1 = torch.mean(F.softmax(torch.matmul(x, x.reshape([batch, h, length])) / h, dim=1), dim=2)
            # attn2 = torch.mean(F.softmax(torch.matmul(x, x.reshape([batch, h, length])) / math.sqrt(h), dim=1), dim=2)
            print("+++++++++attn1:", torch.mean(attn, dim=2)[0, :])
            # print("+++++++++attn2:", attn2[0, :])
